...
export async function generateCompanyBrief(research: PreparedResearchContext, provider: LlmProvider, config: LlmConfig): Promise<CompanyBrief> {
  if (research.company_sources.length === 0) return { summary: "", what_they_do: "", sources: [] };
  const draft = await generateValidated(provider, "company_brief", SYSTEM, JSON.stringify({ sources: research.company_sources }), CompanyBriefDraftSchema, config.timeoutMs, config.model);
